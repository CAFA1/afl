...
EXP_ST void init_forkserver(char** argv) {
	...
	if (!forksrv_pid) {//long: child process
		//long: log
		...
		if (out_file) { //long: @@ .cur_input
		
			dup2(dev_null_fd, 0); //long: stdin is assigned as dev_null
		...
		execv(target_path, argv); //long: execv
	...
	rlen = read(fsrv_st_fd, &status, 4); //long: wait and read status
...
static u8 run_target(char** argv, u32 timeout) {
	...
	if (dumb_mode == 1 || no_forkserver) {
		...
		if ((res = write(fsrv_ctl_fd, &prev_timed_out, 4)) != 4) {//long: start fork server
		...
		if ((res = read(fsrv_st_fd, &child_pid, 4)) != 4) { //long: read child_pid
	...
	if (dumb_mode == 1 || no_forkserver) {
		...
		if ((res = read(fsrv_st_fd, &status, 4)) != 4) { //long: read status
	...
	if (WIFSIGNALED(status) && !stop_soon) {
		...
		if (child_timed_out && kill_signal == SIGKILL) return FAULT_TMOUT;
			...
			return FAULT_CRASH; //long: signal
